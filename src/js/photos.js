...
+function ($) {
  ...
  Photos.prototype = {
      ...
      open: function (index) {
          ...
          var dstIndex = this.config.initIndex;
          if (index !== undefined) {
              dstIndex = index;
          }
          if (dstIndex < 0) dstIndex = 0;
          if (dstIndex > this.config.items.length - 1)
              dstIndex = this.config.items.length - 1;

          this.lastActiveIndex = this.activeIndex = dstIndex;
          this.wrapperTransform = -($(window).width() * dstIndex);
          this.wrapperLastTransform = this.wrapperTransform;
          // this.wrapper.transition(0);
          this.wrapper.transform('translate3d(-' + $(window).width() * dstIndex + 'px,0,0)');
          this.container.find('.caption-item').eq(dstIndex).addClass('active');
          this.container.find('.swiper-pagination-bullet')
              .eq(dstIndex).addClass('swiper-pagination-bullet-active')
              .siblings('.swiper-pagination-bullet-active').removeClass('swiper-pagination-bullet-active');
          this.config.initIndex = dstIndex;
